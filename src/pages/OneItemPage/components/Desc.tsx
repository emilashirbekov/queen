...
const Desc = ({ product }: IDesc) => {
		...
		if (color !== '' && size !== '') {
			...
			if (
				!basket.some(
					//@ts-ignore
					some =>
						some.id === product.id && some.color === color && some.size === size
				)
			) {
				const newItem = {
					id: product.id,
	...
	return (
		<div>
          ...
          <p className='text-[20px] font-normal'>Пожалуйста пройдите регистрацию.</p>
          <Link className='py-[14px] bg-[#212121] w-full text-center rounded-[10px] text-[20px] font-semibold text-white mt-[12px]' to='/register'>зарегистрироватся</Link>
        </div>
			</Modal>
		</div>
	)
