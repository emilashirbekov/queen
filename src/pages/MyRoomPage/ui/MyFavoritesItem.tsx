...
const MyFavoritesItem = () => {
	...
	const handleCheckboxChange = (
		...
		setState(prevItems => {
			if (isChecked) {
				if (prevItems.some(item => item.id === favourite.id)) {
					return prevItems
				} else {
					return [...prevItems, favourite]
				}
			} else {
				return prevItems.filter(item => item.id !== favourite.id)
			}
		})
	}
	...
	return (
		<div className='col-span-4 md:col-span-3 px-[24px] py-[20px]'>
			<div className='flex mb-[25px] justify-end md:justify-between items-center'>
				<h4 className='text-primary font-semibold hidden md:block'>
					Избранное
				</h4>
				...
				{favourites.length !== 0 ? (
					favourites.map(favourite => (
						<div key={favourite.id} className='relative'>
							...
							<Link to={`/catalog/one_item/${favourite.product}`}>
								<img
									className='bg-secondary border border-thirsty w-[184px] min-h-[185px] rounded-[25px]'
									src={favourite.product_image}
									alt='item'
								/>
							</Link>
						</div>
